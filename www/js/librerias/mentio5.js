...
angular.module('mentio5', [])
    .directive('mentio5', ['mentio5Util', '$document', '$compile', '$log', '$timeout',
        function (mentio5Util, $document, $compile, $log, $timeout) {
        return {
            restrict: 'A',
            scope: {
                macros: '=mentio5Macros',
                search: '&mentio5Search',
                select: '&mentio5Select',
                items: '=mentio5Items',
                typedTerm: '=mentio5TypedTerm',
                altId: '=mentio5Id',
                iframeElement: '=mentio5IframeElement',
                requireLeadingSpace: '=mentio5RequireLeadingSpace',
                selectNotFound: '=mentio5SelectNotFound',
                trimTerm: '=mentio5TrimTerm',
                ngModel: '='
            },
            controller: ["$scope", "$timeout", "$attrs", function($scope, $timeout, $attrs) {
                ...
                $scope.bridgeSearch = function(termString) {
                    var searchFn = $attrs.mentio5Search ? $scope.search : $scope.defaultSearch;
                    searchFn({
                        term: termString
                    });
                };
                ...
                $scope.bridgeSelect = function(itemVar) {
                    var selectFn = $attrs.mentio5Select ? $scope.select : $scope.defaultSelect;
                    return selectFn({
                        item: itemVar
                    });
                };
                ...
                $scope.replaceText = function (text, hasTrailingSpace) {
                    ...
                    mentio5Util.replaceTriggerText($scope.context(), $scope.targetElement, $scope.targetElementPath,
                ...
                $scope.replaceMacro = function(macro, hasTrailingSpace) {
                    if (!hasTrailingSpace) {
                        ...
                        $scope.timer = $timeout(function() {
                            mentio5Util.replaceMacroText($scope.context(), $scope.targetElement,
                        ...
                        mentio5Util.replaceMacroText($scope.context(), $scope.targetElement, $scope.targetElementPath,
            ...
            link: function (scope, element, attrs) {
                ...
                if (attrs.mentio5Items) {
                    scope.localItems = [];
                    scope.parentScope = scope;
                    var itemsRef = attrs.mentio5Search ? ' mentio5-items="items"' : ' mentio5-items="localItems"';

                    scope.defaultTriggerChar = attrs.mentio5TriggerChar ? scope.$eval(attrs.mentio5TriggerChar) : '@';

                    var html = '<mentio5-menu' +
                        ' mentio5-search="bridgeSearch(term)"' +
                        ' mentio5-select="bridgeSelect(item)"' +
                        itemsRef;

                    if (attrs.mentio5TemplateUrl) {
                        html = html + ' mentio5-template-url="' + attrs.mentio5TemplateUrl + '"';
                    }
                    html = html + ' mentio5-trigger-char="\'' + scope.defaultTriggerChar + '\'"' +
                        ' mentio5-parent-scope="parentScope"' +
                        '/>';
                ...
                scope.$watch(
                    ...
                    function (newValue) {
                        ...
                        if (scope.triggerCharSet === undefined) {
                            $log.error('Error, no mentio5-items attribute was provided, ' +
                                'and no separate mentio5-menus were specified.  Nothing to do.');
                            return;
                        }
                        ...
                        var mentio5nInfo = mentio5Util.getTriggerInfo(scope.context(), scope.triggerCharSet,
                            scope.requireLeadingSpace, isActive);

                        if (mentio5nInfo !== undefined &&
                                (
                                    !isActive ||
                                    (isActive &&
                                        (
                                            ...
                                            (isContentEditable && mentio5nInfo.mentio5nTriggerChar ===
                                                scope.currentmentio5nTriggerChar) ||
                                            (!isContentEditable && mentio5nInfo.mentio5nPosition ===
                                                scope.currentmentio5nPosition)
                                        )
                                    )
                                )
                            )
                        {
                            ...
                            if (mentio5nInfo.mentio5nSelectedElement) {
                                scope.targetElement = mentio5nInfo.mentio5nSelectedElement;
                                scope.targetElementPath = mentio5nInfo.mentio5nSelectedPath;
                                scope.targetElementSelectedOffset = mentio5nInfo.mentio5nSelectedOffset;
                            }

                            /* publish to external ngModel */
                            scope.setTriggerText(mentio5nInfo.mentio5nText);
                            /* remember current position */
                            scope.currentmentio5nPosition = mentio5nInfo.mentio5nPosition;
                            scope.currentmentio5nTriggerChar = mentio5nInfo.mentio5nTriggerChar;
                            /* perform query */
                            scope.query(mentio5nInfo.mentio5nTriggerChar, mentio5nInfo.mentio5nText);
                        } else {
                            var currentTypedTerm = scope.typedTerm;
                            scope.setTriggerText('');
                            scope.hideAll();

                            var macroMatchInfo = mentio5Util.getMacroMatch(scope.context(), scope.macros);
                            ...
                            if (macroMatchInfo !== undefined) {
                                ...
                                var lastScope = scope.triggerCharMap[scope.currentmentio5nTriggerChar];
                                if (lastScope) {
    ...
    .directive('mentio5Menu', ['mentio5Util', '$rootScope', '$log', '$window', '$document',
        function (mentio5Util, $rootScope, $log, $window, $document) {
        return {
            restrict: 'E',
            scope: {
                search: '&mentio5Search',
                select: '&mentio5Select',
                items: '=mentio5Items',
                triggerChar: '=mentio5TriggerChar',
                forElem: '=mentio5For',
                parentScope: '=mentio5ParentScope'
            },
            templateUrl: function(tElement, tAttrs) {
                return tAttrs.mentio5TemplateUrl !== undefined ? tAttrs.mentio5TemplateUrl : 'mentio5-menu.tpl.html';
            },
            controller: ["$scope", function ($scope) {
                ...
                this.selectItem = $scope.selectItem = function (item) {
                    ...
                    if (typeof text.then === 'function') {
                        /* text is a promise, at least our best guess */
                        text.then($scope.parentmentio5.replaceText);
                    } else {
                        $scope.parentmentio5.replaceText(text);
                    }
                };
            ...
            link: function (scope, element) {
                ...
                if (scope.parentScope) {
                    ...
                    if (!scope.forElem) {
                        $log.error('mentio5-menu requires a target element in tbe mentio5-for attribute');
                        return;
                    }
                    if (!scope.triggerChar) {
                        $log.error('mentio5-menu requires a trigger char');
                        return;
                    }
                    // send own scope to mentio5 directive so that the menu
                    // becomes attached
                ...
                angular.element($window).bind(
                    'resize', function () {
                        if (scope.isVisible()) {
                            ...
                            mentio5Util.popUndermentio5n(scope.parentmentio5.context(),
                ...
                scope.parentmentio5.$on('$destroy', function () {
                    element.remove();
                });
                ...
                scope.adjustScroll = function (direction) {
                    ...
                    var menuItem = (menuEl.querySelector('[mentio5-menu-item].active') || 
                        menuEl.querySelector('[data-mentio5-menu-item].active'));
    ...
    .directive('mentio5MenuItem', function () {
        return {
            restrict: 'A',
            scope: {
                item: '=mentio5MenuItem'
            },
            require: '^mentio5Menu',
    ...
    .filter('mentio5Highlight', function() {
...
angular.module('mentio5')
    .factory('mentio5Util', ["$window", "$location", "$anchorScroll", "$timeout", function ($window, $location, $anchorScroll, $timeout) {

        // public
        function popUndermentio5n (ctx, triggerCharSet, selectionEl, requireLeadingSpace) {
            var coordinates;
            var mentio5nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, false);

            if (mentio5nInfo !== undefined) {

                if (selectedElementIsTextAreaOrInput(ctx)) {
                    coordinates = getTextAreaOrInputUnderlinePosition(ctx, getDocument(ctx).activeElement,
                        mentio5nInfo.mentio5nPosition);
                } else {
                    coordinates = getContentEditableCaretPosition(ctx, mentio5nInfo.mentio5nPosition);
                }
        ...
        function replaceTriggerText (ctx, targetElement, path, offset, triggerCharSet, 
            ...
            var mentio5nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, true, hasTrailingSpace);

            if (mentio5nInfo !== undefined) {
                if (selectedElementIsTextAreaOrInput()) {
                    var myField = getDocument(ctx).activeElement;
                    text = text + ' ';
                    var startPos = mentio5nInfo.mentio5nPosition;
                    var endPos = mentio5nInfo.mentio5nPosition + mentio5nInfo.mentio5nText.length + 1;
                    ...
                    text = text + '\xA0';
                    pasteHtml(ctx, text, mentio5nInfo.mentio5nPosition,
                            mentio5nInfo.mentio5nPosition + mentio5nInfo.mentio5nText.length + 1);
                }
            }
        }
            ...
            if (effectiveRange !== undefined && effectiveRange !== null) {
                    ...
                    )
                {
                    ...
                    if (!leadingSpace && (menuAlreadyActive || !(/[\xA0\s]/g.test(currentTriggerSnippet)))) {
                        return {
                            mentio5nPosition: mostRecentTriggerCharPos,
                            mentio5nText: currentTriggerSnippet,
                            mentio5nSelectedElement: selected,
                            mentio5nSelectedPath: path,
                            mentio5nSelectedOffset: offset,
                            mentio5nTriggerChar: triggerChar
                        };
                    }
                }
            }
        ...
        return {
            // public
            popUndermentio5n: popUndermentio5n,
...
angular.module("mentio5").run(["$templateCache", function($templateCache) {$templateCache.put("mentio5-menu.tpl.html","<style>\n.scrollable-menu {\n    height: auto;\n    max-height: 300px;\n    overflow: auto;\n}\n\n.menu-highlighted {\n    font-weight: bold;\n}\n</style>\n<ul class=\"dropdown-menu scrollable-menu\" style=\"display:block\">\n    <li mentio5-menu-item=\"item\" ng-repeat=\"item in items track by $index\">\n        <a class=\"text-primary\" ng-bind-html=\"item.label | mentio5Highlight:typedTerm:\'menu-highlighted\' | unsafe\"></a>\n    </li>\n</ul>");}]);