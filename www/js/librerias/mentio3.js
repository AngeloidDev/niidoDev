...
angular.module('mentio3', [])
    .directive('mentio3', ['mentio3Util', '$document', '$compile', '$log', '$timeout',
        function (mentio3Util, $document, $compile, $log, $timeout) {
        return {
            restrict: 'A',
            scope: {
                macros: '=mentio3Macros',
                search: '&mentio3Search',
                select: '&mentio3Select',
                items: '=mentio3Items',
                typedTerm: '=mentio3TypedTerm',
                altId: '=mentio3Id',
                iframeElement: '=mentio3IframeElement',
                requireLeadingSpace: '=mentio3RequireLeadingSpace',
                selectNotFound: '=mentio3SelectNotFound',
                trimTerm: '=mentio3TrimTerm',
                ngModel: '='
            },
            controller: ["$scope", "$timeout", "$attrs", function($scope, $timeout, $attrs) {
                ...
                $scope.bridgeSearch = function(termString) {
                    var searchFn = $attrs.mentio3Search ? $scope.search : $scope.defaultSearch;
                    searchFn({
                        term: termString
                    });
                };
                ...
                $scope.bridgeSelect = function(itemVar) {
                    var selectFn = $attrs.mentio3Select ? $scope.select : $scope.defaultSelect;
                    return selectFn({
                        item: itemVar
                    });
                };
                ...
                $scope.replaceText = function (text, hasTrailingSpace) {
                    ...
                    mentio3Util.replaceTriggerText($scope.context(), $scope.targetElement, $scope.targetElementPath,
                ...
                $scope.replaceMacro = function(macro, hasTrailingSpace) {
                    if (!hasTrailingSpace) {
                        ...
                        $scope.timer = $timeout(function() {
                            mentio3Util.replaceMacroText($scope.context(), $scope.targetElement,
                        ...
                        mentio3Util.replaceMacroText($scope.context(), $scope.targetElement, $scope.targetElementPath,
            ...
            link: function (scope, element, attrs) {
                ...
                if (attrs.mentio3Items) {
                    scope.localItems = [];
                    scope.parentScope = scope;
                    var itemsRef = attrs.mentio3Search ? ' mentio3-items="items"' : ' mentio3-items="localItems"';

                    scope.defaultTriggerChar = attrs.mentio3TriggerChar ? scope.$eval(attrs.mentio3TriggerChar) : '@';

                    var html = '<mentio3-menu' +
                        ' mentio3-search="bridgeSearch(term)"' +
                        ' mentio3-select="bridgeSelect(item)"' +
                        itemsRef;

                    if (attrs.mentio3TemplateUrl) {
                        html = html + ' mentio3-template-url="' + attrs.mentio3TemplateUrl + '"';
                    }
                    html = html + ' mentio3-trigger-char="\'' + scope.defaultTriggerChar + '\'"' +
                        ' mentio3-parent-scope="parentScope"' +
                        '/>';
                ...
                scope.$watch(
                    ...
                    function (newValue) {
                        ...
                        if (scope.triggerCharSet === undefined) {
                            $log.error('Error, no mentio3-items attribute was provided, ' +
                                'and no separate mentio3-menus were specified.  Nothing to do.');
                            return;
                        }
                        ...
                        var mentio3nInfo = mentio3Util.getTriggerInfo(scope.context(), scope.triggerCharSet,
                            scope.requireLeadingSpace, isActive);

                        if (mentio3nInfo !== undefined &&
                                (
                                    !isActive ||
                                    (isActive &&
                                        (
                                            ...
                                            (isContentEditable && mentio3nInfo.mentio3nTriggerChar ===
                                                scope.currentmentio3nTriggerChar) ||
                                            (!isContentEditable && mentio3nInfo.mentio3nPosition ===
                                                scope.currentmentio3nPosition)
                                        )
                                    )
                                )
                            )
                        {
                            ...
                            if (mentio3nInfo.mentio3nSelectedElement) {
                                scope.targetElement = mentio3nInfo.mentio3nSelectedElement;
                                scope.targetElementPath = mentio3nInfo.mentio3nSelectedPath;
                                scope.targetElementSelectedOffset = mentio3nInfo.mentio3nSelectedOffset;
                            }

                            /* publish to external ngModel */
                            scope.setTriggerText(mentio3nInfo.mentio3nText);
                            /* remember current position */
                            scope.currentmentio3nPosition = mentio3nInfo.mentio3nPosition;
                            scope.currentmentio3nTriggerChar = mentio3nInfo.mentio3nTriggerChar;
                            /* perform query */
                            scope.query(mentio3nInfo.mentio3nTriggerChar, mentio3nInfo.mentio3nText);
                        } else {
                            var currentTypedTerm = scope.typedTerm;
                            scope.setTriggerText('');
                            scope.hideAll();

                            var macroMatchInfo = mentio3Util.getMacroMatch(scope.context(), scope.macros);
                            ...
                            if (macroMatchInfo !== undefined) {
                                ...
                                var lastScope = scope.triggerCharMap[scope.currentmentio3nTriggerChar];
                                if (lastScope) {
    ...
    .directive('mentio3Menu', ['mentio3Util', '$rootScope', '$log', '$window', '$document',
        function (mentio3Util, $rootScope, $log, $window, $document) {
        return {
            restrict: 'E',
            scope: {
                search: '&mentio3Search',
                select: '&mentio3Select',
                items: '=mentio3Items',
                triggerChar: '=mentio3TriggerChar',
                forElem: '=mentio3For',
                parentScope: '=mentio3ParentScope'
            },
            templateUrl: function(tElement, tAttrs) {
                return tAttrs.mentio3TemplateUrl !== undefined ? tAttrs.mentio3TemplateUrl : 'mentio3-menu.tpl.html';
            },
            controller: ["$scope", function ($scope) {
                ...
                this.selectItem = $scope.selectItem = function (item) {
                    ...
                    if (typeof text.then === 'function') {
                        /* text is a promise, at least our best guess */
                        text.then($scope.parentmentio3.replaceText);
                    } else {
                        $scope.parentmentio3.replaceText(text);
                    }
                };
            ...
            link: function (scope, element) {
                ...
                if (scope.parentScope) {
                    ...
                    if (!scope.forElem) {
                        $log.error('mentio3-menu requires a target element in tbe mentio3-for attribute');
                        return;
                    }
                    if (!scope.triggerChar) {
                        $log.error('mentio3-menu requires a trigger char');
                        return;
                    }
                    // send own scope to mentio3 directive so that the menu
                    // becomes attached
                ...
                angular.element($window).bind(
                    'resize', function () {
                        if (scope.isVisible()) {
                            ...
                            mentio3Util.popUndermentio3n(scope.parentmentio3.context(),
                ...
                scope.parentmentio3.$on('$destroy', function () {
                    element.remove();
                });
                ...
                scope.adjustScroll = function (direction) {
                    ...
                    var menuItem = (menuEl.querySelector('[mentio3-menu-item].active') ||
                        menuEl.querySelector('[data-mentio3-menu-item].active'));
    ...
    .directive('mentio3MenuItem', function () {
        return {
            restrict: 'A',
            scope: {
                item: '=mentio3MenuItem'
            },
            require: '^mentio3Menu',
    ...
    .filter('mentio3Highlight', function() {
...
angular.module('mentio3')
    .factory('mentio3Util', ["$window", "$location", "$anchorScroll", "$timeout", function ($window, $location, $anchorScroll, $timeout) {

        // public
        function popUndermentio3n (ctx, triggerCharSet, selectionEl, requireLeadingSpace) {
            var coordinates;
            var mentio3nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, false);

            if (mentio3nInfo !== undefined) {

                if (selectedElementIsTextAreaOrInput(ctx)) {
                    coordinates = getTextAreaOrInputUnderlinePosition(ctx, getDocument(ctx).activeElement,
                        mentio3nInfo.mentio3nPosition);
                } else {
                    coordinates = getContentEditableCaretPosition(ctx, mentio3nInfo.mentio3nPosition);
                }
        ...
        function replaceTriggerText (ctx, targetElement, path, offset, triggerCharSet,
            ...
            var mentio3nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, true, hasTrailingSpace);

            if (mentio3nInfo !== undefined) {
                if (selectedElementIsTextAreaOrInput()) {
                    var myField = getDocument(ctx).activeElement;
                    text = text + ' ';
                    var startPos = mentio3nInfo.mentio3nPosition;
                    var endPos = mentio3nInfo.mentio3nPosition + mentio3nInfo.mentio3nText.length + 1;
                    ...
                    text = text + '\xA0';
                    pasteHtml(ctx, text, mentio3nInfo.mentio3nPosition,
                            mentio3nInfo.mentio3nPosition + mentio3nInfo.mentio3nText.length + 1);
                }
            }
        }
            ...
            if (effectiveRange !== undefined && effectiveRange !== null) {
                    ...
                    )
                {
                    ...
                    if (!leadingSpace && (menuAlreadyActive || !(/[\xA0\s]/g.test(currentTriggerSnippet)))) {
                        return {
                            mentio3nPosition: mostRecentTriggerCharPos,
                            mentio3nText: currentTriggerSnippet,
                            mentio3nSelectedElement: selected,
                            mentio3nSelectedPath: path,
                            mentio3nSelectedOffset: offset,
                            mentio3nTriggerChar: triggerChar
                        };
                    }
                }
            }
        ...
        return {
            // public
            popUndermentio3n: popUndermentio3n,
...
angular.module("mentio3").run(["$templateCache", function($templateCache) {$templateCache.put("mentio3-menu.tpl.html","<style>\n.scrollable-menu {\n    height: auto;\n    max-height: 300px;\n    overflow: auto;\n}\n\n.menu-highlighted {\n    font-weight: bold;\n}\n</style>\n<ul class=\"dropdown-menu scrollable-menu\" style=\"display:block\">\n    <li mentio3-menu-item=\"item\" ng-repeat=\"item in items track by $index\">\n        <a class=\"text-primary\" ng-bind-html=\"item.label | mentio3Highlight:typedTerm:\'menu-highlighted\' | unsafe\"></a>\n    </li>\n</ul>");}]);
