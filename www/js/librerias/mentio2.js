...
angular.module('mentio2', [])
    .directive('mentio2', ['mentio2Util', '$document', '$compile', '$log', '$timeout',
        function (mentio2Util, $document, $compile, $log, $timeout) {
        return {
            restrict: 'A',
            scope: {
                macros: '=mentio2Macros',
                search: '&mentio2Search',
                select: '&mentio2Select',
                items: '=mentio2Items',
                typedTerm: '=mentio2TypedTerm',
                altId: '=mentio2Id',
                iframeElement: '=mentio2IframeElement',
                requireLeadingSpace: '=mentio2RequireLeadingSpace',
                selectNotFound: '=mentio2SelectNotFound',
                trimTerm: '=mentio2TrimTerm',
                ngModel: '='
            },
            controller: ["$scope", "$timeout", "$attrs", function($scope, $timeout, $attrs) {
                ...
                $scope.bridgeSearch = function(termString) {
                    var searchFn = $attrs.mentio2Search ? $scope.search : $scope.defaultSearch;
                    searchFn({
                        term: termString
                    });
                };
                ...
                $scope.bridgeSelect = function(itemVar) {
                    var selectFn = $attrs.mentio2Select ? $scope.select : $scope.defaultSelect;
                    return selectFn({
                        item: itemVar
                    });
                };
                ...
                $scope.replaceText = function (text, hasTrailingSpace) {
                    ...
                    mentio2Util.replaceTriggerText($scope.context(), $scope.targetElement, $scope.targetElementPath,
                ...
                $scope.replaceMacro = function(macro, hasTrailingSpace) {
                    if (!hasTrailingSpace) {
                        ...
                        $scope.timer = $timeout(function() {
                            mentio2Util.replaceMacroText($scope.context(), $scope.targetElement,
                        ...
                        mentio2Util.replaceMacroText($scope.context(), $scope.targetElement, $scope.targetElementPath,
            ...
            link: function (scope, element, attrs) {
                ...
                if (attrs.mentio2Items) {
                    scope.localItems = [];
                    scope.parentScope = scope;
                    var itemsRef = attrs.mentio2Search ? ' mentio2-items="items"' : ' mentio2-items="localItems"';

                    scope.defaultTriggerChar = attrs.mentio2TriggerChar ? scope.$eval(attrs.mentio2TriggerChar) : '@';

                    var html = '<mentio2-menu' +
                        ' mentio2-search="bridgeSearch(term)"' +
                        ' mentio2-select="bridgeSelect(item)"' +
                        itemsRef;

                    if (attrs.mentio2TemplateUrl) {
                        html = html + ' mentio2-template-url="' + attrs.mentio2TemplateUrl + '"';
                    }
                    html = html + ' mentio2-trigger-char="\'' + scope.defaultTriggerChar + '\'"' +
                        ' mentio2-parent-scope="parentScope"' +
                        '/>';
                ...
                scope.$watch(
                    ...
                    function (newValue) {
                        ...
                        if (scope.triggerCharSet === undefined) {
                            $log.error('Error, no mentio2-items attribute was provided, ' +
                                'and no separate mentio2-menus were specified.  Nothing to do.');
                            return;
                        }
                        ...
                        var mentio2nInfo = mentio2Util.getTriggerInfo(scope.context(), scope.triggerCharSet,
                            scope.requireLeadingSpace, isActive);

                        if (mentio2nInfo !== undefined &&
                                (
                                    !isActive ||
                                    (isActive &&
                                        (
                                            ...
                                            (isContentEditable && mentio2nInfo.mentio2nTriggerChar ===
                                                scope.currentmentio2nTriggerChar) ||
                                            (!isContentEditable && mentio2nInfo.mentio2nPosition ===
                                                scope.currentmentio2nPosition)
                                        )
                                    )
                                )
                            )
                        {
                            ...
                            if (mentio2nInfo.mentio2nSelectedElement) {
                                scope.targetElement = mentio2nInfo.mentio2nSelectedElement;
                                scope.targetElementPath = mentio2nInfo.mentio2nSelectedPath;
                                scope.targetElementSelectedOffset = mentio2nInfo.mentio2nSelectedOffset;
                            }

                            /* publish to external ngModel */
                            scope.setTriggerText(mentio2nInfo.mentio2nText);
                            /* remember current position */
                            scope.currentmentio2nPosition = mentio2nInfo.mentio2nPosition;
                            scope.currentmentio2nTriggerChar = mentio2nInfo.mentio2nTriggerChar;
                            /* perform query */
                            scope.query(mentio2nInfo.mentio2nTriggerChar, mentio2nInfo.mentio2nText);
                        } else {
                            var currentTypedTerm = scope.typedTerm;
                            scope.setTriggerText('');
                            scope.hideAll();

                            var macroMatchInfo = mentio2Util.getMacroMatch(scope.context(), scope.macros);
                            ...
                            if (macroMatchInfo !== undefined) {
                                ...
                                var lastScope = scope.triggerCharMap[scope.currentmentio2nTriggerChar];
                                if (lastScope) {
    ...
    .directive('mentio2Menu', ['mentio2Util', '$rootScope', '$log', '$window', '$document',
        function (mentio2Util, $rootScope, $log, $window, $document) {
        return {
            restrict: 'E',
            scope: {
                search: '&mentio2Search',
                select: '&mentio2Select',
                items: '=mentio2Items',
                triggerChar: '=mentio2TriggerChar',
                forElem: '=mentio2For',
                parentScope: '=mentio2ParentScope'
            },
            templateUrl: function(tElement, tAttrs) {
                return tAttrs.mentio2TemplateUrl !== undefined ? tAttrs.mentio2TemplateUrl : 'mentio2-menu.tpl.html';
            },
            controller: ["$scope", function ($scope) {
                ...
                this.selectItem = $scope.selectItem = function (item) {
                    ...
                    if (typeof text.then === 'function') {
                        /* text is a promise, at least our best guess */
                        text.then($scope.parentmentio2.replaceText);
                    } else {
                        $scope.parentmentio2.replaceText(text);
                    }
                };
            ...
            link: function (scope, element) {
                ...
                if (scope.parentScope) {
                    ...
                    if (!scope.forElem) {
                        $log.error('mentio2-menu requires a target element in tbe mentio2-for attribute');
                        return;
                    }
                    if (!scope.triggerChar) {
                        $log.error('mentio2-menu requires a trigger char');
                        return;
                    }
                    // send own scope to mentio2 directive so that the menu
                    // becomes attached
                ...
                angular.element($window).bind(
                    'resize', function () {
                        if (scope.isVisible()) {
                            ...
                            mentio2Util.popUndermentio2n(scope.parentmentio2.context(),
                ...
                scope.parentmentio2.$on('$destroy', function () {
                    element.remove();
                });
                ...
                scope.adjustScroll = function (direction) {
                    ...
                    var menuItem = (menuEl.querySelector('[mentio2-menu-item].active') || 
                        menuEl.querySelector('[data-mentio2-menu-item].active'));
    ...
    .directive('mentio2MenuItem', function () {
        return {
            restrict: 'A',
            scope: {
                item: '=mentio2MenuItem'
            },
            require: '^mentio2Menu',
    ...
    .filter('mentio2Highlight', function() {
...
angular.module('mentio2')
    .factory('mentio2Util', ["$window", "$location", "$anchorScroll", "$timeout", function ($window, $location, $anchorScroll, $timeout) {

        // public
        function popUndermentio2n (ctx, triggerCharSet, selectionEl, requireLeadingSpace) {
            var coordinates;
            var mentio2nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, false);

            if (mentio2nInfo !== undefined) {

                if (selectedElementIsTextAreaOrInput(ctx)) {
                    coordinates = getTextAreaOrInputUnderlinePosition(ctx, getDocument(ctx).activeElement,
                        mentio2nInfo.mentio2nPosition);
                } else {
                    coordinates = getContentEditableCaretPosition(ctx, mentio2nInfo.mentio2nPosition);
                }
        ...
        function replaceTriggerText (ctx, targetElement, path, offset, triggerCharSet, 
            ...
            var mentio2nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, true, hasTrailingSpace);

            if (mentio2nInfo !== undefined) {
                if (selectedElementIsTextAreaOrInput()) {
                    var myField = getDocument(ctx).activeElement;
                    text = text + ' ';
                    var startPos = mentio2nInfo.mentio2nPosition;
                    var endPos = mentio2nInfo.mentio2nPosition + mentio2nInfo.mentio2nText.length + 1;
                    ...
                    text = text + '\xA0';
                    pasteHtml(ctx, text, mentio2nInfo.mentio2nPosition,
                            mentio2nInfo.mentio2nPosition + mentio2nInfo.mentio2nText.length + 1);
                }
            }
        }
            ...
            if (effectiveRange !== undefined && effectiveRange !== null) {
                    ...
                    )
                {
                    ...
                    if (!leadingSpace && (menuAlreadyActive || !(/[\xA0\s]/g.test(currentTriggerSnippet)))) {
                        return {
                            mentio2nPosition: mostRecentTriggerCharPos,
                            mentio2nText: currentTriggerSnippet,
                            mentio2nSelectedElement: selected,
                            mentio2nSelectedPath: path,
                            mentio2nSelectedOffset: offset,
                            mentio2nTriggerChar: triggerChar
                        };
                    }
                }
            }
        ...
        return {
            // public
            popUndermentio2n: popUndermentio2n,
...
angular.module("mentio2").run(["$templateCache", function($templateCache) {$templateCache.put("mentio2-menu.tpl.html","<style>\n.scrollable-menu {\n    height: auto;\n    max-height: 300px;\n    overflow: auto;\n}\n\n.menu-highlighted {\n    font-weight: bold;\n}\n</style>\n<ul class=\"dropdown-menu scrollable-menu\" style=\"display:block\">\n    <li mentio2-menu-item=\"item\" ng-repeat=\"item in items track by $index\">\n        <a class=\"text-primary\" ng-bind-html=\"item.label | mentio2Highlight:typedTerm:\'menu-highlighted\' | unsafe\"></a>\n    </li>\n</ul>");}]);