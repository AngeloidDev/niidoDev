...
angular.module('mentio4', [])
    .directive('mentio4', ['mentio4Util', '$document', '$compile', '$log', '$timeout',
        function (mentio4Util, $document, $compile, $log, $timeout) {
        return {
            restrict: 'A',
            scope: {
                macros: '=mentio4Macros',
                search: '&mentio4Search',
                select: '&mentio4Select',
                items: '=mentio4Items',
                typedTerm: '=mentio4TypedTerm',
                altId: '=mentio4Id',
                iframeElement: '=mentio4IframeElement',
                requireLeadingSpace: '=mentio4RequireLeadingSpace',
                selectNotFound: '=mentio4SelectNotFound',
                trimTerm: '=mentio4TrimTerm',
                ngModel: '='
            },
            controller: ["$scope", "$timeout", "$attrs", function($scope, $timeout, $attrs) {
                ...
                $scope.bridgeSearch = function(termString) {
                    var searchFn = $attrs.mentio4Search ? $scope.search : $scope.defaultSearch;
                    searchFn({
                        term: termString
                    });
                };
                ...
                $scope.bridgeSelect = function(itemVar) {
                    var selectFn = $attrs.mentio4Select ? $scope.select : $scope.defaultSelect;
                    return selectFn({
                        item: itemVar
                    });
                };
                ...
                $scope.replaceText = function (text, hasTrailingSpace) {
                    ...
                    mentio4Util.replaceTriggerText($scope.context(), $scope.targetElement, $scope.targetElementPath,
                ...
                $scope.replaceMacro = function(macro, hasTrailingSpace) {
                    if (!hasTrailingSpace) {
                        ...
                        $scope.timer = $timeout(function() {
                            mentio4Util.replaceMacroText($scope.context(), $scope.targetElement,
                        ...
                        mentio4Util.replaceMacroText($scope.context(), $scope.targetElement, $scope.targetElementPath,
            ...
            link: function (scope, element, attrs) {
                ...
                if (attrs.mentio4Items) {
                    scope.localItems = [];
                    scope.parentScope = scope;
                    var itemsRef = attrs.mentio4Search ? ' mentio4-items="items"' : ' mentio4-items="localItems"';

                    scope.defaultTriggerChar = attrs.mentio4TriggerChar ? scope.$eval(attrs.mentio4TriggerChar) : '@';

                    var html = '<mentio4-menu' +
                        ' mentio4-search="bridgeSearch(term)"' +
                        ' mentio4-select="bridgeSelect(item)"' +
                        itemsRef;

                    if (attrs.mentio4TemplateUrl) {
                        html = html + ' mentio4-template-url="' + attrs.mentio4TemplateUrl + '"';
                    }
                    html = html + ' mentio4-trigger-char="\'' + scope.defaultTriggerChar + '\'"' +
                        ' mentio4-parent-scope="parentScope"' +
                        '/>';
                ...
                scope.$watch(
                    ...
                    function (newValue) {
                        ...
                        if (scope.triggerCharSet === undefined) {
                            $log.error('Error, no mentio4-items attribute was provided, ' +
                                'and no separate mentio4-menus were specified.  Nothing to do.');
                            return;
                        }
                        ...
                        var mentio4nInfo = mentio4Util.getTriggerInfo(scope.context(), scope.triggerCharSet,
                            scope.requireLeadingSpace, isActive);

                        if (mentio4nInfo !== undefined &&
                                (
                                    !isActive ||
                                    (isActive &&
                                        (
                                            ...
                                            (isContentEditable && mentio4nInfo.mentio4nTriggerChar ===
                                                scope.currentmentio4nTriggerChar) ||
                                            (!isContentEditable && mentio4nInfo.mentio4nPosition ===
                                                scope.currentmentio4nPosition)
                                        )
                                    )
                                )
                            )
                        {
                            ...
                            if (mentio4nInfo.mentio4nSelectedElement) {
                                scope.targetElement = mentio4nInfo.mentio4nSelectedElement;
                                scope.targetElementPath = mentio4nInfo.mentio4nSelectedPath;
                                scope.targetElementSelectedOffset = mentio4nInfo.mentio4nSelectedOffset;
                            }

                            /* publish to external ngModel */
                            scope.setTriggerText(mentio4nInfo.mentio4nText);
                            /* remember current position */
                            scope.currentmentio4nPosition = mentio4nInfo.mentio4nPosition;
                            scope.currentmentio4nTriggerChar = mentio4nInfo.mentio4nTriggerChar;
                            /* perform query */
                            scope.query(mentio4nInfo.mentio4nTriggerChar, mentio4nInfo.mentio4nText);
                        } else {
                            var currentTypedTerm = scope.typedTerm;
                            scope.setTriggerText('');
                            scope.hideAll();

                            var macroMatchInfo = mentio4Util.getMacroMatch(scope.context(), scope.macros);
                            ...
                            if (macroMatchInfo !== undefined) {
                                ...
                                var lastScope = scope.triggerCharMap[scope.currentmentio4nTriggerChar];
                                if (lastScope) {
    ...
    .directive('mentio4Menu', ['mentio4Util', '$rootScope', '$log', '$window', '$document',
        function (mentio4Util, $rootScope, $log, $window, $document) {
        return {
            restrict: 'E',
            scope: {
                search: '&mentio4Search',
                select: '&mentio4Select',
                items: '=mentio4Items',
                triggerChar: '=mentio4TriggerChar',
                forElem: '=mentio4For',
                parentScope: '=mentio4ParentScope'
            },
            templateUrl: function(tElement, tAttrs) {
                return tAttrs.mentio4TemplateUrl !== undefined ? tAttrs.mentio4TemplateUrl : 'mentio4-menu.tpl.html';
            },
            controller: ["$scope", function ($scope) {
                ...
                this.selectItem = $scope.selectItem = function (item) {
                    ...
                    if (typeof text.then === 'function') {
                        /* text is a promise, at least our best guess */
                        text.then($scope.parentmentio4.replaceText);
                    } else {
                        $scope.parentmentio4.replaceText(text);
                    }
                };
            ...
            link: function (scope, element) {
                ...
                if (scope.parentScope) {
                    ...
                    if (!scope.forElem) {
                        $log.error('mentio4-menu requires a target element in tbe mentio4-for attribute');
                        return;
                    }
                    if (!scope.triggerChar) {
                        $log.error('mentio4-menu requires a trigger char');
                        return;
                    }
                    // send own scope to mentio4 directive so that the menu
                    // becomes attached
                ...
                angular.element($window).bind(
                    'resize', function () {
                        if (scope.isVisible()) {
                            ...
                            mentio4Util.popUndermentio4n(scope.parentmentio4.context(),
                ...
                scope.parentmentio4.$on('$destroy', function () {
                    element.remove();
                });
                ...
                scope.adjustScroll = function (direction) {
                    ...
                    var menuItem = (menuEl.querySelector('[mentio4-menu-item].active') || 
                        menuEl.querySelector('[data-mentio4-menu-item].active'));
    ...
    .directive('mentio4MenuItem', function () {
        return {
            restrict: 'A',
            scope: {
                item: '=mentio4MenuItem'
            },
            require: '^mentio4Menu',
    ...
    .filter('mentio4Highlight', function() {
...
angular.module('mentio4')
    .factory('mentio4Util', ["$window", "$location", "$anchorScroll", "$timeout", function ($window, $location, $anchorScroll, $timeout) {

        // public
        function popUndermentio4n (ctx, triggerCharSet, selectionEl, requireLeadingSpace) {
            var coordinates;
            var mentio4nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, false);

            if (mentio4nInfo !== undefined) {

                if (selectedElementIsTextAreaOrInput(ctx)) {
                    coordinates = getTextAreaOrInputUnderlinePosition(ctx, getDocument(ctx).activeElement,
                        mentio4nInfo.mentio4nPosition);
                } else {
                    coordinates = getContentEditableCaretPosition(ctx, mentio4nInfo.mentio4nPosition);
                }
        ...
        function replaceTriggerText (ctx, targetElement, path, offset, triggerCharSet, 
            ...
            var mentio4nInfo = getTriggerInfo(ctx, triggerCharSet, requireLeadingSpace, true, hasTrailingSpace);

            if (mentio4nInfo !== undefined) {
                if (selectedElementIsTextAreaOrInput()) {
                    var myField = getDocument(ctx).activeElement;
                    text = text + ' ';
                    var startPos = mentio4nInfo.mentio4nPosition;
                    var endPos = mentio4nInfo.mentio4nPosition + mentio4nInfo.mentio4nText.length + 1;
                    ...
                    text = text + '\xA0';
                    pasteHtml(ctx, text, mentio4nInfo.mentio4nPosition,
                            mentio4nInfo.mentio4nPosition + mentio4nInfo.mentio4nText.length + 1);
                }
            }
        }
            ...
            if (effectiveRange !== undefined && effectiveRange !== null) {
                    ...
                    )
                {
                    ...
                    if (!leadingSpace && (menuAlreadyActive || !(/[\xA0\s]/g.test(currentTriggerSnippet)))) {
                        return {
                            mentio4nPosition: mostRecentTriggerCharPos,
                            mentio4nText: currentTriggerSnippet,
                            mentio4nSelectedElement: selected,
                            mentio4nSelectedPath: path,
                            mentio4nSelectedOffset: offset,
                            mentio4nTriggerChar: triggerChar
                        };
                    }
                }
            }
        ...
        return {
            // public
            popUndermentio4n: popUndermentio4n,
...
angular.module("mentio4").run(["$templateCache", function($templateCache) {$templateCache.put("mentio4-menu.tpl.html","<style>\n.scrollable-menu {\n    height: auto;\n    max-height: 300px;\n    overflow: auto;\n}\n\n.menu-highlighted {\n    font-weight: bold;\n}\n</style>\n<ul class=\"dropdown-menu scrollable-menu\" style=\"display:block\">\n    <li mentio4-menu-item=\"item\" ng-repeat=\"item in items track by $index\">\n        <a class=\"text-primary\" ng-bind-html=\"item.label | mentio4Highlight:typedTerm:\'menu-highlighted\' | unsafe\"></a>\n    </li>\n</ul>");}]);